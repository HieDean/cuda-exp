                              ...
                              int bs, int m, int n, int k)
{
    ...
    for (int kk = 0; kk < k; kk += TILEK)
    {
        ...
        for (int ii = 0; ii < TILEK; ii += blockLayoutRowB)
        {
            ...
            {
                FLOAT4(sharedTileB[physical_rowIdB][physical_colIdB]) =
            ...
            else
            {
                for (int fi = 0; fi < 4; ++fi)
                {
                    ...
                    int offset = fi << 5;
                    sharedTileB[physical_rowIdB][colIdB + offset] =
        ...
        for (int tk = 0; tk < TILEK; ++tk)
        {
            ...
            {
                FLOAT4(regB[ii / blockLayoutColC]) = FLOAT4(sharedTileB[tk][ii + (colIdC << 2)]);
            }
