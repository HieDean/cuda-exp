...
void helper(KernelFunc func,
            ...
            float *attnWeights, float *O, int bs, int num_heads, int seq_len, int head_dim, cudaStream_t stream,
            int warmup, int run,
            const std::vector<float> &attn_weights, const std::vector<float> &o,
            std::string tag)
{
    // first time, only run for correctness check
    checkCudaErrors(cudaMemsetAsync(attnWeights, 0, attn_weights.size() * sizeof(float), stream));
    checkCudaErrors(cudaMemsetAsync(O, 0, o.size() * sizeof(float), stream));
    func(Q, K, V, attnMasks, attnWeights, O, bs, num_heads, seq_len, head_dim, stream);
    checkCudaErrors(cudaStreamSynchronize(stream));

    // check diff
    std::vector<float> attn_weights_from_device(attn_weights.size());
    checkCudaErrors(cudaMemcpy(attn_weights_from_device.data(), attnWeights,
                               attn_weights.size() * sizeof(float), cudaMemcpyDeviceToHost));
    check_difference(attn_weights, attn_weights_from_device, tag + "_attn_weights");
...
int main(int argc, char **argv)
{
    ...
    std::vector<float> attn_weights, o;
    ...
    for (int ii = 0; ii < bs * num_heads * seq_len * seq_len; ++ii)
    {
        attn_weights.push_back(uniform_dist(random_engine));
    }
    ...
    checkCudaErrors(cudaMalloc(reinterpret_cast<void **>(&attnWeights), attn_weights.size() * sizeof(float)));
    checkCudaErrors(cudaMemsetAsync(attnWeights, 0, attn_weights.size() * sizeof(float), stream));
